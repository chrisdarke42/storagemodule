(function ($, Drupal, drupalSettings) {
 ...
 //           { name: "Shared File Services", matches: [1,2,4,5,6,7] }
    ...
    // load the services
    $.getJSON( "/rest/testservicelist", function( responseb ) {
        services = responseb;
        ...
        $.each(responseb, function (ind, val) {
            ...
            if ((categories.indexOf(val.summary) === -1)  && (val.summary != "Help text for any service")) {
                categories.push(val.summary);
            }
        });
...
// instead of labels, we have field_data[field]["label"]
        ...
        if (selected_vendor.length > 0) {
            $(".service-panel").each( function () {
                var service_title = $(this).find('.service-title').text();
                if (service_title == selected_vendor) {
                    //$(this).find(".cardcheckbox").prop('checked', true);
                    $(this).find(".cardcheckbox").click();
                }
            });
    ...
    function evaluate_services(){
        ...
        for (i=0;i<servicelist.length;i++) {
            ...
            // hide services which don't match selected facets
...
// the question highlighting only works if the control key is pressed :-)
    ...
    /*scroll To See More functions to add notification if scroll bar is shown
        ...
        css located at storage/themes/custom/rdmsg/css/customizations.css
    */
