    ...
    private function createservicelist() {
        ...
        foreach ($nodes as $node) {
            ...
            if ($paragraph) {
                ...
                $s["Example Use"] = $paragraph->get("field_example_use")->getValue()[0]["value"];
                $s["Cost"] = $paragraph->get("field_cost")->getValue()[0]["value"];
                $s["Capacity"] = $paragraph->get("field_capacity")->getValue()[0]["value"];
                $s["Access and Collaboration"] =
                    $paragraph->get("field_access_and_collaboration")->getValue()[0]["value"];
                $s["Data Allowed"] = $paragraph->get("field_data_allowed")->getValue()[0]["value"];
                $s["Durability"] = $paragraph->get("field_durability")->getValue()[0]["value"];
                $s["Complexity"] = $paragraph->get("field_complexity")->getValue()[0]["value"];
                $s["Contact"] = $paragraph->get("field_contact")->getValue()[0]["value"];
            }
...
// this is how to get the node info
 ...
 //       ->load("paragraph" . '.' . "service_paragraphs" . '.' . "default");
    ...
    public function configuration() {
        ...
        $config = \Drupal::service('config.factory')->getEditable("storage.settings");
        $data = [];
